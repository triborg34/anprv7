...
# Load YOLO models
model_plate = torch.hub.load('yolov5', 'custom', 'model/plateYolo.onnx', source='local', device=device, force_reload=True,skip_validation=True)
# model_char = torch.hub.load('yolov5', 'custom', 'model/CharsYolo.pt', source='local', device=device, force_reload=True,skip_validation=True)
model_char = torch.hub.load('yolov5', 'custom', 'model/CharsYolo.pt', source='local', device=device, force_reload=True,skip_validation=True)
...
def detect_plate_chars(cropped_plate):
    ...
    # Resize cropped plate to match model input size
